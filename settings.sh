...
_update_cron_retention() {
    local s3_config="$1" vps_ip="$2" username="$3" ssh_key="$4"
    ...
    # Mettre a jour le fichier local (garder endpoint/bucket/keys, changer cron/retention)
    local cur_endpoint cur_bucket cur_access cur_secret
    cur_endpoint=$(read_state_var "$s3_config" "S3_ENDPOINT")
    cur_bucket=$(read_state_var "$s3_config" "S3_BUCKET")
    cur_access=$(read_state_var "$s3_config" "S3_ACCESS_KEY")
    cur_secret=$(read_state_var "$s3_config" "S3_SECRET_KEY")
...
sudo -u "$USERNAME" crontab -l 2>/dev/null | grep -v "backup-cron.sh" | sudo -u "$USERNAME" crontab - 2>/dev/null || true

if [ -n "$CRON_EXPR" ] && [ -f "$CRON_SCRIPT" ]; then
    # Mettre a jour la retention dans le script existant
    if [ "$(uname)" = "Linux" ]; then
        sed -i "s/^RETENTION=.*/RETENTION=\"$RETENTION\"/" "$CRON_SCRIPT"
    fi
    # Reinstaller le cron
    (sudo -u "$USERNAME" crontab -l 2>/dev/null; echo "$CRON_EXPR $CRON_SCRIPT >> /home/$USERNAME/backup-cron.log 2>&1") | sudo -u "$USERNAME" crontab -
    echo "[OK] Cron mis a jour."
elif [ -n "$CRON_EXPR" ]; then
    echo "[WARN] Script backup-cron.sh introuvable. Relancez la configuration complete."
else
    echo "[OK] Cron desactive."
fi
CRON_UPDATE_EOF
...
setup_remote_backup() {
    ...
    # Si une config existe deja, afficher le sous-menu
    if [ -f "$s3_config" ]; then
        local cur_endpoint cur_bucket cur_cron cur_retention
        cur_endpoint=$(read_state_var "$s3_config" "S3_ENDPOINT")
        cur_bucket=$(read_state_var "$s3_config" "S3_BUCKET")
        cur_cron=$(read_state_var "$s3_config" "S3_CRON_SCHEDULE")
        cur_retention=$(read_state_var "$s3_config" "S3_RETENTION_DAYS")

        # Traduire le label cron
        local cron_display="$cur_cron"
        case "$cur_cron" in
            daily) cron_display="$MSG_SETTINGS_RBACKUP_CRON_DAILY_LABEL" ;;
            weekly) cron_display="$MSG_SETTINGS_RBACKUP_CRON_WEEKLY_LABEL" ;;
            none) cron_display="$MSG_SETTINGS_RBACKUP_CRON_NONE_LABEL" ;;
        esac

        echo "  $MSG_SETTINGS_RBACKUP_CURRENT_CONFIG"
        printf "  $MSG_SETTINGS_RBACKUP_CURRENT_ENDPOINT\n" "$cur_endpoint"
        printf "  $MSG_SETTINGS_RBACKUP_CURRENT_BUCKET\n" "$cur_bucket"
        printf "  $MSG_SETTINGS_RBACKUP_CURRENT_CRON\n" "$cron_display"
        printf "  $MSG_SETTINGS_RBACKUP_CURRENT_RETENTION\n" "$cur_retention"
        echo ""
        echo -e "  ${GREEN}1)${NC} $MSG_SETTINGS_RBACKUP_MENU_TEST"
        echo -e "  ${GREEN}2)${NC} $MSG_SETTINGS_RBACKUP_MENU_BACKUP_NOW"
        echo -e "  ${GREEN}3)${NC} $MSG_SETTINGS_RBACKUP_MENU_RECONFIG"
        echo -e "  ${GREEN}4)${NC} $MSG_SETTINGS_RBACKUP_MENU_CRON"
        echo -e "  ${GREEN}5)${NC} $MSG_SETTINGS_RBACKUP_MENU_BACK"
        echo ""
        local menu_choice
        read -p "$MSG_SETTINGS_RBACKUP_MENU_PROMPT" menu_choice

        case "$menu_choice" in
            1)
                # Tester la connexion via SSH
                echo ""
                info "$MSG_SETTINGS_RBACKUP_TESTING"
                if ssh -i "$ssh_key" -o BatchMode=yes "${username}@${vps_ip}" "rclone lsf s3backup:$(read_state_var "$s3_config" "S3_BUCKET") --max-depth 0" &>/dev/null; then
                    success "$MSG_SETTINGS_RBACKUP_TEST_OK"
                else
                    err "$MSG_SETTINGS_RBACKUP_TEST_FAIL"
                fi
                echo ""
                return
                ;;
            2)
                # Lancer backup.sh
                echo ""
                info "$MSG_SETTINGS_RBACKUP_BACKUP_RUNNING"
                _backup_path="$(dirname "$0")/backup.sh"
                if [ ! -f "$_backup_path" ]; then
                    _backup_path=$(mktemp)
                    _CLEANUP_FILES+=("$_backup_path")
                    if ! curl -fsSL "https://raw.githubusercontent.com/mariusdjen/vpskit/main/backup.sh" -o "$_backup_path" 2>/dev/null; then
                        err "$MSG_SETTINGS_RBACKUP_DOWNLOAD_FAILED"
                        rm -f "$_backup_path"
                        return
                    fi
                fi
                bash "$_backup_path" -ip "$vps_ip" -key "$ssh_key" -user "$username" --cloud
                return
                ;;
            3)
                # Continuer vers le wizard de reconfiguration (ci-dessous)
                echo ""
                ;;
            4)
                # Modifier uniquement cron + retention
                _update_cron_retention "$s3_config" "$vps_ip" "$username" "$ssh_key"
                return
                ;;
            *)
                return
    ...
    # Configuration du stockage (wizard complet)
    ...
    # Nettoyer l'endpoint : retirer trailing slash et nom du bucket si present
...
# Tester la connexion
if sudo -u "$USERNAME" rclone lsf "s3backup:$S3_BUCKET" --max-depth 0 &>/dev/null; then
...
main() {
    # Support appel direct : bash settings.sh --remote-backup
    if [ "${1:-}" = "--remote-backup" ]; then
        setup_remote_backup
        exit 0
    fi
